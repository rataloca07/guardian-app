...
export class MonitorPage implements OnInit, OnDestroy {
  ...
  ionViewWillEnter() {
    ...
    }, 50000);  // Actualización cada 10 segundos
  }
...
export class MonitorPage implements OnInit, OnDestroy {
  ...
  ionViewWillEnter() {
    // Cada vez que se acceda a la vista de monitoreo, se inicia la actualización automática cada 50 segundos
    this.obtenerPaciente();
    this.obtenerZonasSeguras();  // Asegurar que las zonas seguras se actualicen
    this.intervalId = setInterval(() => {
      this.actualizarMonitoreo();
    }, 5000);  // Actualización cada 50 segundos
  }
  ...
  async obtenerPaciente() {
    const pacienteId = await this.authService.obtenerPacienteId();  // Obtener el ID del paciente de manera dinámica
    this.pacienteService.obtenerPaciente(pacienteId).subscribe((data) => {
      this.paciente = data;  // Actualizar el objeto paciente
      this.ritmoCardiaco = data.ritmoCardiaco;  // Actualizar el ritmo cardíaco

      // Actualizar la posición del marcador en el mapa
      if (this.marker) {
        this.marker.setLatLng([data.latitud, data.longitud]); // Actualizar la latitud y longitud del marcador
        this.map.setView([data.latitud, data.longitud], 16);  // Centrar el mapa en la nueva ubicación
      } else {
        ...
        //this.marker = L.marker([data.latitud, data.longitud]).addTo(this.map);
        this.marker = L.marker([data.latitud, data.longitud],{icon:iconoMarcador}).addTo(this.map);
        this.map.setView([data.latitud, data.longitud], 16);  // Centrar el mapa en la ubicación del paciente
      }
      
      // Actualizar el título en la interfaz (si corresponde)
      // Puedes agregar esta línea si necesitas actualizar la vista de la UI
      // this.tituloPaciente = data.nombre; // Si tienes una variable para el título del paciente
    });
  }
